...
function RiderRow({ a }: { a: AdminRiderApplication }) {
...
function RidersManager() {
  ...
  const rows = (q.data ?? []).filter((a) => a.status === tab);
  return (
    <>
      ...
      {rows.length ? <div className="mt-4 divide-y divide-border">{rows.map((a) => <RiderRow key={a.id} a={a} />)}</div>
